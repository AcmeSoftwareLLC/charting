import 'package:acme_chart/src/core/interactive_layer/crosshair/find.dart';
...
void main() {
  group('findClosestToEpoch should', () {
    test('return null if list is empty', () {
      expect(findClosestToEpoch(100, []), equals(null));
    });
    test('return tick with exact epoch if present', () {
      ...
      expect(
        findClosestToEpoch(100, [tick100, tick200, tick300]),
        equals(tick100),
      );
      expect(
        findClosestToEpoch(200, [tick100, tick200, tick300]),
        equals(tick200),
      );
      expect(
        findClosestToEpoch(300, [tick100, tick200, tick300]),
        equals(tick300),
      );
    });
    test('return tick with closest epoch if exact isn\'t present', () {
      ...
      expect(
        findClosestToEpoch(101, [tick100, tick110, tick120, tick130, tick140]),
        equals(tick100),
      );
    });
  });
  ...
  group('findEpochIndex should', () {
    test('throw exception if list is empty', () {
      expect(() => findEpochIndex(0, <Tick>[]), throwsArgumentError);
    });
