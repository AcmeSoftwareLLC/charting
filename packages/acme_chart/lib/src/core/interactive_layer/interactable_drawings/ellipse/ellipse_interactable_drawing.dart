...
import '../../../../add_ons/drawing_tools_ui/ellipse/ellipse_drawing_tool_config.dart';
...
import 'ellipse_adding_preview_desktop.dart';
import 'ellipse_adding_preview_mobile.dart';

/// Interactable drawing implementation for ellipse drawing tool.
///
/// Handles rendering, hit testing, and drag interactions for ellipses.
/// Supports dragging individual corners of the bounding box (to resize) or
/// the whole ellipse (to move it), with visual feedback matching the other
/// interactable drawings.
class EllipseInteractableDrawing
    extends InteractableDrawing<EllipseDrawingToolConfig> {
  /// Initializes [EllipseInteractableDrawing].
  EllipseInteractableDrawing({
    required EllipseDrawingToolConfig config,
  ...
  /// Start corner of the ellipse's bounding box.
  EdgePoint? startPoint;

  /// End corner of the ellipse's bounding box (diagonally opposite to
  /// [startPoint]).
  EdgePoint? endPoint;

  /// Tracks which corner is being dragged, if any.
  ///
  /// [null]: dragging the whole ellipse.
  ///
  ...
  bool hitTest(Offset offset, EpochToX epochToX, QuoteToY quoteToY) {
    ...
    return _isOnEllipseBoundary(
      Rect.fromPoints(startOffset, endOffset),
      offset,
    );
  }

  /// Checks whether [position] is near the stroke boundary of the ellipse
  /// inscribed in [rect].
  bool _isOnEllipseBoundary(Rect rect, Offset position) {
    ...
    final double lineWidth = config.lineStyle.thickness;
    const double touchTolerance = 10;

    final Offset delta = position - rect.center;
    final double distanceFromCenter = delta.distance;
    if (distanceFromCenter == 0) {
      return false;
    }

    final double cosTheta = delta.dx / distanceFromCenter;
    final double sinTheta = delta.dy / distanceFromCenter;
    final double inverseRadiusSquared =
        (cosTheta * cosTheta) / (radiusX * radiusX) +
        (sinTheta * sinTheta) / (radiusY * radiusY);
    final double boundaryRadiusAtAngle = 1 / sqrt(inverseRadiusSquared);

    final double distanceFromBoundary =
        (distanceFromCenter - boundaryRadiusAtAngle).abs();

    return distanceFromBoundary <= lineWidth + touchTolerance;
  }
  ...
  void paint(
    ...
    if (config.pattern == DrawingPatterns.solid) {
      ...
      if (drawingState.contains(DrawingToolState.selected)) {
        ...
        canvas.drawOval(rect, neonPaint);
      }
    }
  ...
  @override
  EllipseDrawingToolConfig getUpdatedConfig() =>
