...
import '../../../../add_ons/drawing_tools_ui/rectangle/rectangle_drawing_tool_config.dart';
...
/// Interactable drawing implementation for rectangle drawing tool.
///
/// Handles rendering, hit testing, and drag interactions for rectangles.
/// Supports dragging individual corners (to resize) or the whole rectangle
/// (to move it), with visual feedback matching the other interactable
/// drawings.
class RectangleInteractableDrawing
  ...
  /// End corner of the rectangle (diagonally opposite to [startPoint]).
  EdgePoint? endPoint;
  ...
  bool hitTest(Offset offset, EpochToX epochToX, QuoteToY quoteToY) {
    ...
    return _isOnRectangleBoundary(
      Rect.fromPoints(startOffset, endOffset),
      offset,
    );
  }

  /// Checks whether [position] is on the stroke boundary of [rect].
  bool _isOnRectangleBoundary(Rect rect, Offset position) {
    final double lineWidth = config.lineStyle.thickness;
    const double touchTolerance = 10;

    final List<Rect> boundaries = <Rect>[
      Rect.fromLTWH(
      ...
      Rect.fromLTWH(
        rect.right - lineWidth - touchTolerance,
        rect.top - touchTolerance,
        lineWidth + touchTolerance * 2,
        rect.height + touchTolerance * 2,
      ),
      Rect.fromLTWH(
        rect.left - touchTolerance,
        rect.bottom - lineWidth - touchTolerance,
        rect.width + touchTolerance * 2,
        lineWidth + touchTolerance * 2,
      ),
    ];

    return boundaries.any((Rect bound) => bound.contains(position));
  }
