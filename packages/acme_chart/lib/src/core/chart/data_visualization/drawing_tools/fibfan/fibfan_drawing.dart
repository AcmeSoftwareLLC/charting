...
import '../../../../../core/chart/data_visualization/drawing_tools/data_model/draggable_edge_point.dart';
...
import '../../../../../core/chart/data_visualization/drawing_tools/data_model/point.dart';
import '../../../../../core/chart/data_visualization/drawing_tools/drawing.dart';
import '../../../../../core/chart/data_visualization/drawing_tools/drawing_data.dart';
import '../../../../../core/chart/data_visualization/drawing_tools/fibfan/label.dart';
...
class FibfanDrawing extends Drawing with LineVectorDrawingMixin {
  ...
  bool isVectorHit(Vector vector, Offset position, LineStyle lineStyle) {
    final double lineLength = sqrt(
      pow(vector.y1 - vector.y0, 2) + pow(vector.x1 - vector.x0, 2),
    );
    ...
    final double distance =
        ((vector.y1 - vector.y0) * position.dx -
            (vector.x1 - vector.x0) * position.dy +
            vector.x1 * vector.y0 -
            vector.y1 * vector.x0) /
        sqrt(pow(vector.y1 - vector.y0, 2) + pow(vector.x1 - vector.x0, 2));

    final double xDistToStart = position.dx - vector.x0;
    final double yDistToStart = position.dy - vector.y0;

    /// Limit the detection to start and end point of the line
    final double dotProduct =
        (xDistToStart * (vector.x1 - vector.x0) +
            yDistToStart * (vector.y1 - vector.y0)) /
        lineLength;
  ...
  Path getTrianglePath(Vector startVector, Vector endVector) => Path()
  ...
  void _drawTriangle(
    ...
    canvas.drawPath(
      path,
      paint.fillPaintStyle(fillStyle.color, fillStyle.thickness),
    );
  }
  ...
  void onPaint(
    ...
    final Paint linePaintStyle = paint.linePaintStyle(
      lineStyle.color,
      lineStyle.thickness,
    );
    ...
    if (drawingPart == DrawingParts.marker) {
      ...
      /// Draw the shaded area between two vectors
      Vector getLineVectorWithEnd(double endXCoord, double startQuoteToY) =>
          getLineVector(
            startXCoord,
            startQuoteToY,
            endXCoord,
      ...
      const double initialVectorDegree = 0.618;

      /// Defines the degree of middle inner vector from the  base vector
      const double middleVectorDegree = 0.5;

      /// Defines the degree of final inner vector from the base vector
      const double finalVectorDegree = 0.382;

      /// Add vectors
      _zeroDegreeVector = getLineVectorWithEnd(endXCoord, endQuoteToY);
      _initialInnerVector = getLineVectorWithEnd(
        endXCoord,
        ((endQuoteToY - startQuoteToY) * initialVectorDegree) + startQuoteToY,
      );

      _middleInnerVector = getLineVectorWithEnd(
        endXCoord,
        ((endQuoteToY - startQuoteToY) * middleVectorDegree) + startQuoteToY,
      );
      _finalInnerVector = getLineVectorWithEnd(
        endXCoord,
        ((endQuoteToY - startQuoteToY) * finalVectorDegree) + startQuoteToY,
      );
      _baseVector = getLineVectorWithEnd(endXCoord, startQuoteToY);
      ...
      /// Draw labels
      Label(startXCoord: startXCoord.toInt(), endXCoord: endXCoord.toInt())
        ..drawLabel(
          canvas,
          size,
          lineStyle,
  ...
  bool hitTest(
    ...
    bool isVectorPosisionHit(Vector vector) =>
    ...
    return isVectorPosisionHit(_baseVector) ||
        isVectorPosisionHit(_finalInnerVector) ||
        isVectorPosisionHit(_middleInnerVector) ||
        isVectorPosisionHit(_initialInnerVector) ||
        isVectorPosisionHit(_zeroDegreeVector) ||
