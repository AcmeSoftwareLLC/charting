...
import '../../../../../core/chart/data_visualization/chart_series/line_series/channel_fill_painter.dart';
...
class ADXSeries extends Series {
  ...
  SeriesPainter<Series>? createPainter() {
    ...
    if (config.showShading) {
      return ChannelFillPainter(
        positiveDISeries,
        negativeDISeries,
        firstUpperChannelFillColor: config.positiveLineStyle.color.withValues(
          alpha: 0.2,
        ),
        secondUpperChannelFillColor: config.negativeLineStyle.color.withValues(
          alpha: 0.2,
        ),
      );
    }
  ...
  bool didUpdate(ChartData? oldData) {
    ...
    final bool positiveDIUpdated = positiveDISeries.didUpdate(
      series?.positiveDISeries,
    );
    final bool negativeDIUpdated = negativeDISeries.didUpdate(
      series?.negativeDISeries,
    );
    final bool adxUpdated = adxSeries.didUpdate(series?.adxSeries);
    final bool histogramUpdated = adxHistogramSeries.didUpdate(
      series?.adxHistogramSeries,
    );
