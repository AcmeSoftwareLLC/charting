...
class CandleIndicatorPainter extends HorizontalBarrierPainter<CandleIndicator> {
  /// Initializes [series].
  CandleIndicatorPainter(super.series);
  ...
  void onPaint({
    ...
    if (series.showTimer) {
      _paintTimer(canvas, size, quoteToY, animationInfo);
    }
  ...
  void _paintTimer(
    ...
    final double timerWidth = max<double>(
      timerPainter.width,
      valuePainter.width,
    );
    ...
    final Rect labelArea = Rect.fromCenter(
      center: Offset(
        size.width - rightMargin - padding - timerWidth / 2,
    ...
    // Label.
    paintLabelBackground(canvas, labelArea, LabelShape.rectangle, _paint);
