...
import '../../../core/chart/helpers/functions/conversion.dart';
...
class XAxisModel extends ChangeNotifier {
  ...
  XAxisModel({
    ...
    _maxCurrentTickOffset = maxCurrentTickOffset;
    _defaultTickOffset =
        defaultTickOffset?.clamp(0, _maxCurrentTickOffset) ??
  ...
  void _updateEntries(List<Tick>? entries) {
    ...
    if (firstLoad || reload) {
      ...
      final List<Tick> prefix = entries.sublist(
        0,
        entries.length - _entries!.length + 1,
      );
  ...
  void _fitData() {
    if (width != null && (_entries?.isNotEmpty ?? false)) {
      ...
      _msPerPx = (msDataDuration / pxTargetDataWidth).clamp(
        _minMsPerPx,
        _maxMsPerPx,
      );
  ...
  double xFromEpochSnapped(int epoch) {
    ...
    final int clampedEpoch = epoch.clamp(
      leftBoundEpoch - granularity,
      rightBoundEpoch + granularity,
    );
  ...
  void scrollToLastTick({bool animate = true}) {
    ...
    _scrollAnimationController
      ..value = 0
      ..animateTo(distance, curve: Curves.easeOut, duration: duration);
  }
  ...
  void _clampRightBoundEpoch() {
    if (_minRightBoundEpoch <= _maxRightBoundEpoch) {
      _rightBoundEpoch = _rightBoundEpoch.clamp(
        _minRightBoundEpoch,
        _maxRightBoundEpoch,
      );
    }
  ...
  List<DateTime> getNoOverlapGridTimestamps() {
    ...
    final List<DateTime> gridTimestampList = gridTimestamps(
      timeGridInterval: timeGridInterval(
        pxFromMs,
        minDistanceBetweenLines: minDistanceBetweenTimeGridLines,
      ),
