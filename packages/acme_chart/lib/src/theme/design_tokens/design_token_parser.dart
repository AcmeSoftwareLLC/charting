...
///
/// Returns:
///   A Map containing the configuration settings for token processing.
///
/// Throws:
  ...
  try {
    ...
    if (!file.existsSync()) {
      throw const FileSystemException(
        ...
        DesignTokenUtils.tokensJsonPath,
      );
    }
    ...
    try {
      ...
      throw FileSystemException(
        'Error reading tokens file: ${e.message}',
        e.path,
        e.osError,
      );
    }
    ...
    try {
      ...
      throw FormatException(
        'Invalid JSON format in ${file.path}: ${e.message}',
        e.source,
      );
    }
    ...
    if (tokenCategories.containsKey('dark')) {
      try {
        _processTokenCategory(
          tokens,
          tokenCategories['dark'],
          DesignTokenUtils.categoryDark,
          darkThemeTokenValues,
        );
        successCount++;
      } on Exception catch (e) {
    ...
    if (tokenCategories.containsKey('component')) {
      try {
        _processTokenCategory(
          ...
          DesignTokenUtils.categoryComponent,
          componentTokenValues,
        );
        successCount++;
      } on Exception catch (e) {
...
void _processTokenCategory(
  ...
  try {
    ...
    try {
      generator = DesignTokenGeneratorFactory.createGenerator(
        category,
        className,
      );
    } catch (e) {
...
void _testNumericTokenReferences() {
  ...
  final String numericTokenRef = numericTokenInput.substring(
    1,
    numericTokenInput.length - 1,
  );
...
void _testCubicBezierParser() {
  ...
  try {
    ...
    final String cubicBezierTransformed = formatter
    ...
    final String cubicBezierTransformed2 = formatter
