### How to use AcmeChart chart component

`AcmeChart` is a wrapper around the `chart` widget that provides the ability to add/remove indicators and manage saving/restoring added ones in storage.
...
We can also implement our own interface to do all the above and integrate it with the `AcmeChart` component.
...
After creating the instance you can pass it to the AcmeChart component.
...
`AddOnsRepository` is a subtype of `ChangeNotifier` so when we call this method, later `AcmeChart` will be informed about the change in the list of indicators.
