...
/// First support pivot level: `(2 * pivot) - previousHigh`.
///
...
/// [UnimplementedError].
class PivotS1Indicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  PivotS1Indicator(super.input, {PivotPointsFn? pivotPoints})
  ...
  T calculate(int index) => throw UnimplementedError(
    'PivotS1Indicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _pivotPoints,
    (pivotPoints) => pivotPoints(entries.highs, entries.lows, entries.closes).s1,
  );
}
