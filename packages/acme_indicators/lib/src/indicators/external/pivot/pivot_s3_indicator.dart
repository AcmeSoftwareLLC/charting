...
/// Third support pivot level: `previousLow - 2 * (previousHigh - pivot)`.
///
...
/// [UnimplementedError].
class PivotS3Indicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  PivotS3Indicator(super.input, {PivotPointsFn? pivotPoints})
  ...
  T calculate(int index) => throw UnimplementedError(
    'PivotS3Indicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _pivotPoints,
    (pivotPoints) => pivotPoints(entries.highs, entries.lows, entries.closes).s3,
  );
}
