...
/// Second resistance pivot level: `pivot + (previousHigh - previousLow)`.
///
...
/// [UnimplementedError].
class PivotR2Indicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  PivotR2Indicator(super.input, {PivotPointsFn? pivotPoints})
  ...
  T calculate(int index) => throw UnimplementedError(
    'PivotR2Indicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _pivotPoints,
    (pivotPoints) => pivotPoints(entries.highs, entries.lows, entries.closes).r2,
  );
}
