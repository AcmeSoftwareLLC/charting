...
/// First resistance pivot level: `(2 * pivot) - previousLow`.
///
...
/// [UnimplementedError].
class PivotR1Indicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  PivotR1Indicator(super.input, {PivotPointsFn? pivotPoints})
  ...
  T calculate(int index) => throw UnimplementedError(
    'PivotR1Indicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _pivotPoints,
    (pivotPoints) => pivotPoints(entries.highs, entries.lows, entries.closes).r1,
  );
}
