...
/// Second support pivot level: `pivot - (previousHigh - previousLow)`.
///
...
/// [UnimplementedError].
class PivotS2Indicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  PivotS2Indicator(super.input, {PivotPointsFn? pivotPoints})
  ...
  T calculate(int index) => throw UnimplementedError(
    'PivotS2Indicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _pivotPoints,
    (pivotPoints) => pivotPoints(entries.highs, entries.lows, entries.closes).s2,
  );
}
