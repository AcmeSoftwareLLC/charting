...
/// Fractal-based swing-low (valley) detection: whether the entry at an index
/// is lower than [strength] neighbors on either side.
///
...
/// [UnimplementedError].
class ValleyIndicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  ValleyIndicator(super.input, {this.strength = 2, PeakValleyFn? peakValley})
  ...
  T calculate(int index) => throw UnimplementedError(
    'ValleyIndicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _peakValley,
    (peakValley) => peakValley(entries.highs, entries.lows, strength).isValley,
  );
}
