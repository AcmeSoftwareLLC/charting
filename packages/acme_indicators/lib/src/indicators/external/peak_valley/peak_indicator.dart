...
/// Fractal-based swing-high (peak) detection: whether the entry at an index
/// is higher than [strength] neighbors on either side.
///
...
/// [UnimplementedError].
class PeakIndicator<T extends IndicatorResult> extends CachedIndicator<T> {
  /// Initializes.
  PeakIndicator(super.input, {this.strength = 2, PeakValleyFn? peakValley})
  ...
  T calculate(int index) => throw UnimplementedError(
    'PeakIndicator has no per-bar Dart implementation. Supply a '
  ...
  List<T> calculateValues() => calculateValuesWith(
    _peakValley,
    (peakValley) => peakValley(entries.highs, entries.lows, strength).isPeak,
  );
}
