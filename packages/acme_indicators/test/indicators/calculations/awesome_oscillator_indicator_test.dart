...
void main() {
  group('Awesome Oscillator Indicator', () {
    ...
    test(
      ...
      () {
        final AwesomeOscillatorIndicator<MockResult> aoIndicator =
            AwesomeOscillatorIndicator<MockResult>(MockInput(ticks));

        expect(aoIndicator.getValue(0).quote, 0.0);
        expect(aoIndicator.getValue(1).quote, 0.0);
        expect(aoIndicator.getValue(2).quote, 0.0);
        expect(aoIndicator.getValue(3).quote, 0.0);
        expect(aoIndicator.getValue(4).quote, 0.0);
      },
    );
    ...
    test('Calculating using custom Period for SMA', () {
      ...
      expect(aoIndicator.getValue(0).quote, 0.0);
      expect(aoIndicator.getValue(1).quote, 0.0);
      expect(aoIndicator.getValue(2).quote, 0.16666666666666607);
      expect(aoIndicator.getValue(3).quote, 1);
      expect(aoIndicator.getValue(4).quote, -3);
    });
    ...
    test(
      ...
      () {
        ...
        final AwesomeOscillatorIndicator<MockResult> aoIndicator1 =
            AwesomeOscillatorIndicator<MockResult>(
              MockInput(ticks),
              smaPeriodOne: 2,
              smaPeriodTwo: 3,
        ...
        final AwesomeOscillatorIndicator<MockResult> aoIndicator2 =
            AwesomeOscillatorIndicator<MockResult>(
                MockInput(ticks2),
                smaPeriodOne: 2,
                smaPeriodTwo: 3,
              )
              ..copyValuesFrom(aoIndicator1)
              ..refreshValueFor(4);

        // Their result in index 3 should be the same since we've copied the result.
        expect(aoIndicator1.getValue(3).quote, aoIndicator2.getValue(3).quote);

        // Calculated result for index 4 is different because the last data is changed.
        expect(roundDouble(aoIndicator1.getValue(4).quote, 2), -3.0);
        expect(roundDouble(aoIndicator2.getValue(4).quote, 2), -2.83);
      },
    );
  });
}
